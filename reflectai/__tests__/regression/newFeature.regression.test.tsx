...
import NewSessionPage from "@/app/nueva-sesion/page";
...
function buildCompletedResponse() {
  return buildSessionResponse({
    title: "Pausa ante una conversacion dificil",
    status: "completed",
    completed_at: "2026-05-12T10:30:00.000Z",
    ai_analysis: {
      summary: "Resumen de regresion",
      recommendation: "Recomendacion de regresion",
...
async function renderWizard() {
  ...
  return screen.findByPlaceholderText(/Escribe/i);
}
...
describe("New feature regression - version 2026-05-12", () => {
  it("RG-01 preserves the grounding flow for high intensity and completes the session", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText(/Escribe/i),
      "Tuve una conversacion complicada en el trabajo.",
    );
    await user.click(screen.getByRole("button", { name: /Siguiente/i }));

    await screen.findByPlaceholderText(/Me dije/i);
    await user.type(
      screen.getByPlaceholderText(/Me dije/i),
      "No estan tomando en serio mi esfuerzo.",
    );
    ...
    expect(
      await screen.findByText(/emocion se siente muy intensa/i),
    ).toBeInTheDocument();

    await user.click(
      screen.getByRole("button", { name: /Estoy listo para continuar/i }),
    );
    ...
    await user.type(screen.getByPlaceholderText(/Mis acciones/i), "Mi tono y mi pausa.");
    await user.type(
    ...
    await screen.findByPlaceholderText(/Una perspectiva alternativa/i);
    await user.type(
      screen.getByPlaceholderText(/Una perspectiva alternativa/i),
      "Puedo responder con calma y pedir claridad.",
    );
    await user.click(screen.getByRole("button", { name: /Finalizar/i }));

    expect(await screen.findByText(/guardada/i)).toBeInTheDocument();
  ...
  it("RG-03 shows the recoverable draft alert and preserves the sessionId", async () => {
      ...
      data: [
        {
          ...
          payload: {
            ...
            responses: [{ id: "Q1_SIT", text: "Borrador con avance" }],
          },
          ai_analysis: {},
        },
        {
    ...
    expect(screen.getByRole("button", { name: /Continuar/i })).toHaveAttribute(
      "href",
      "/nueva-sesion?sessionId=draft-new",
    );
  });
  ...
  it("RG-04 hides empty drafts or drafts older than the latest completed session", async () => {
    listReflectionSessionsMock.mockResolvedValue({
      data: [
        {
          id: "completed-new",
          title: "Sesion completada reciente",
          status: "completed",
        ...
        },
        {
          ...
          payload: {
            ...
            responses: [{ id: "Q1_SIT", text: "Borrador viejo" }],
          },
          ai_analysis: {},
        },
      ],
