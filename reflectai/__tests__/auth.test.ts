...
describe('Validaciones de Registro', () => {
  it('1. Debe pasar la validacion si todos los datos son correctos', () => {
    const resultado = registerSchema.safeParse(usuarioValido);
    expect(resultado.success).toBe(true);
  });

  it('2. Debe fallar si el nombre esta vacio', () => {
  ...
  it('3. Debe fallar si el nombre contiene numeros o simbolos', () => {
    const resultado = registerSchema.safeParse({ ...usuarioValido, firstName: 'Artur0!' });
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('El nombre solo puede contener letras');
    }
  });

  it('4. Debe pasar si el apellido esta vacio', () => {
    const resultado = registerSchema.safeParse({ ...usuarioValido, lastName: '' });
    expect(resultado.success).toBe(true);
  });

  it('5. Debe fallar si el apellido tiene numeros', () => {
    const resultado = registerSchema.safeParse({ ...usuarioValido, lastName: 'Cuevas 2' });
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('El apellido solo puede contener letras');
    }
  });
  ...
  it('5.1. Debe fallar si el apellido excede los 120 caracteres', () => {
    ...
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('Se alcanzó el límite');
    }
  });

  it('6. Debe fallar si el correo no tiene formato valido', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
      email: 'correoinvalido.com',
    });
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('Ingresa un correo válido');
    }
  });
  ...
  it('7. Debe fallar si los correos no coinciden', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
      confirmEmail: 'otro@correo.com',
    });
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('Los correos electrónicos no coinciden');
    }
  });
  ...
  it('9. Debe fallar si la contraseña no tiene numeros', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
  ...
  it('9.1. Debe fallar si la contraseña no tiene mayusculas', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
      password: 'passwordfuerte123!',
      confirmPassword: 'passwordfuerte123!',
  ...
  it('9.2. Debe fallar si la contraseña no tiene minusculas', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
      password: 'PASSWORDFUERTE123!',
      confirmPassword: 'PASSWORDFUERTE123!',
  ...
  it('9.3. Debe fallar si la contraseña no tiene caracter especial', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
      password: 'PasswordFuerte123',
      confirmPassword: 'PasswordFuerte123',
  ...
  it('10. Debe fallar si las contraseñas no coinciden', () => {
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
      confirmPassword: 'PasswordFuerte321!',
    });
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
  ...
  it('11. Debe fallar si la fecha de nacimiento esta vacia', () => {
    const resultado = registerSchema.safeParse({ ...usuarioValido, birthDate: '' });
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('La fecha de nacimiento es obligatoria');
    }
  });
  ...
  it('14. Debe fallar si la contraseña excede más de 64 caracteres', () => {
    const passwordLarga = 'A1!' + 'a'.repeat(62);
    const resultado = registerSchema.safeParse({
      ...usuarioValido,
...
describe('Validaciones de Login', () => {
  ...
  it('17. Debe fallar si la contraseña del login está vacía', () => {
    ...
    expect(resultado.success).toBe(false);
    if (!resultado.success) {
      expect(resultado.error.issues[0].message).toBe('La contraseña es obligatoria');
    }
  });
});
