...
function textareaClass(hasError: boolean) {
  return `w-full min-h-[100px] p-4 mt-2 bg-white/30 backdrop-blur-sm rounded-2xl outline-none transition-all resize-none text-slate-700 placeholder:text-slate-400 ${
    hasError ? "border-2 border-red-400" : "border border-white/60 focus:ring-2 focus:ring-indigo-300/50"
  }`;
}
    ...
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-right-4 duration-500">
      <section className="flex flex-col gap-2">
        ...
        <p className="text-sm text-slate-500 font-medium">
          Ej. protegerme, alertarme, buscar justicia o mantener el control.
        </p>
        <Controller
          name="proposito"
          control={control}
          render={({ field }) => (
            <textarea
              ...
              className={textareaClass(shouldShowPropositoError)}
            />
          )}
        />
        <div className="flex justify-between items-start px-2">
          <span className="text-xs font-bold text-red-500">
            {shouldShowPropositoError ? (errors.proposito?.message as string) : ""}
          </span>
          <span className="text-xs font-bold text-slate-400">
            {valProposito.length}/3000
          </span>
        </div>
      </section>
        ...
        <Controller
          ...
          render={({ field }) => (
            <textarea
              ...
              className={textareaClass(shouldShowControlMioError)}
            />
          )}
        />
        <div className="flex justify-between items-start px-2">
          <span className="text-xs font-bold text-red-500">
            {shouldShowControlMioError ? (errors.controlMio?.message as string) : ""}
          </span>
          <span className="text-xs font-bold text-slate-400">
            {valControlMio.length}/3000
          </span>
        </div>
      </section>
        ...
        <Controller
          ...
          render={({ field }) => (
            <textarea
              ...
              className={textareaClass(shouldShowControlOtrosError)}
            />
          )}
        />
        <div className="flex justify-between items-start px-2">
          <span className="text-xs font-bold text-red-500">
            {shouldShowControlOtrosError ? (errors.controlOtros?.message as string) : ""}
          </span>
          <span className="text-xs font-bold text-slate-400">
            {valControlOtros.length}/3000
          </span>
        </div>
      </section>
