...
function buildResumeState(response: Awaited<ReturnType<typeof getReflectionSession>>): ResumeState {
  ...
  const wasGroundingCompleted = payload.responses.some(
    (responseItem) => responseItem.id === 'SYS_GROUNDING',
  );
...
export function useReflectionWizard(): UseReflectionWizardResult {
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      ...
      try {
        ...
        setFormError(getWizardErrorMessage(error, 'No se pudo iniciar la sesion'));
      } finally {
        if (isMounted) {
          setIsCreatingSession(false);
        }
  ...
  const handleGroundingContinue = useCallback(async () => {
    ...
    try {
      ...
      setFormError(getWizardErrorMessage(error, 'No se pudo guardar el aterrizaje'));
    } finally {
      setIsSaving(false);
    }
  ...
  const handleFinalSubmit = useCallback(async () => {
    ...
    try {
      ...
      setFormError(getWizardErrorMessage(error, 'No se pudo completar la sesion'));
    } finally {
      setIsSaving(false);
    }
